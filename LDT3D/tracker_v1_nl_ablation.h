...
public:
	...
	enum { MAX_POINTS_PER_LINE3 = 3 };
	struct ScanLine
	{
		float     y;
		Point2f   xdir;
		Point2f   xstart;
		ContourPoint  vPoints[MAX_POINTS_PER_LINE3];
		int       nPoints;
...
public:
	...
	public:
		...
		void operator()(ScanLine& line, const float* data, int size, int gaussWindowSizeHalf)
		{
			...
			int max_nlm = nlm;
			float new_alpha = exp(-max_nlm - 1.0f) + 0.125f;
			if (nlm > MAX_POINTS_PER_LINE3)
			{
				std::sort(vlm, vlm + nlm, [](const LocalMaxima& a, const LocalMaxima& b) {
					return a.val > b.val;
					});
				nlm = MAX_POINTS_PER_LINE3;
...
public:
	...
	enum { MAX_POINTS_PER_LINE9 = 9 };
	struct ScanLine
	{
		float     y;
		Point2f   xdir;
		Point2f   xstart;
		ContourPoint  vPoints[MAX_POINTS_PER_LINE9];
		int       nPoints;
		short* cpIndex; //index of the closest contour point for each x position
	public:
		void setCoordinates(const Point2f& start, const Point2f& end, float y)
...
public:
	...
	public:
		...
		void operator()(ScanLine& line, const float* data, int size, int gaussWindowSizeHalf)
		{
			...
			int max_nlm = nlm;
			float new_alpha = exp(-max_nlm - 1.0f) + 0.125f;
			if (nlm > MAX_POINTS_PER_LINE9)
			{
				std::sort(vlm, vlm + nlm, [](const LocalMaxima& a, const LocalMaxima& b) {
					return a.val > b.val;
					});
				nlm = MAX_POINTS_PER_LINE9;
...
public:
	...
	float pro(const Matx33f& K, Pose& pose, const Mat1f& curProb, float thetaT, float errT)
	{
		...
		//return this->pro_naive(K, pose, curProb, thetaT, errT);
		//return this->pro_naive_R3(K, pose, curProb, thetaT, errT);
		//return this->pro_GP(K, pose, curProb, thetaT, errT);
		//return this->pro_GP_PP(K, pose, curProb, thetaT, errT);
	}

	float pro1(const Matx33f& K, Pose& pose, const Mat1f& curProb, float thetaT, float errT);

	float pro_naive(const Matx33f& K, Pose& pose, const Mat1f& curProb, float thetaT, float errT);

	float pro_naive_R3(const Matx33f& K, Pose& pose, const Mat1f& curProb, float thetaT, float errT);

	float pro_GP(const Matx33f& K, Pose& pose, const Mat1f& curProb, float thetaT, float errT);

	float pro_GP_PP(const Matx33f& K, Pose& pose, const Mat1f& curProb, float thetaT, float errT);
};
...
inline float Templates::pro1(const Matx33f& K, Pose& pose, const Mat1f& curProb, float thetaT, float errT)
{
	...
	rectAppend(roi, dW, dW, dW, dW);
	roi = rectOverlapped(roi, Rect(0, 0, curProb.cols, curProb.rows));
	dfr9.computeScanLines(curProb, roi);
	Optimizer9::PoseData dpose9;
	static_cast<Pose&>(dpose9) = dpose;
	for (int itr = 0; itr < outerItrs; ++itr)
	{
		curView = this->_getNearestView(dpose9.R, dpose9.t);
		if (!dfr9.update(dpose9, K, this->views[curView].contourPoints3d, innerItrs, alpha, eps))
			break;
	}
	pose = dpose9;
...
inline float Templates::pro_GP_PP(const Matx33f& K, Pose& pose, const Mat1f& curProb, float thetaT, float errT)
{
	Rect curROI;
	int curView = this->_getNearestView(pose.R, pose.t);
	{
	...
	//if (errMin > errT)
	{
		...
		for (int y = -N; y <= N; ++y)
			for (int x = -N; x <= N; ++x)
			{
				auto dR = theta2OutofplaneRotation(float(x) * dc, float(y) * dc);
...
inline float Templates::pro_naive(const Matx33f& K, Pose& pose, const Mat1f& curProb, float thetaT, float errT)
{
	Rect curROI;
	int curView = this->_getNearestView(pose.R, pose.t);
	{
...
inline float Templates::pro_naive_R3(const Matx33f& K, Pose& pose, const Mat1f& curProb, float thetaT, float errT)
{
	Rect curROI;
	int curView = this->_getNearestView(pose.R, pose.t);
	{
		...
		for (int y = -N; y <= N; ++y)
			for (int x = -N; x <= N; ++x)
				for (int z = -N; z <= N; ++z)
				{
					...
					auto _dRin = cvrm::rotate(float(z) * dc, Vec3f(0.f, 0.f, 1.f));
					Matx33f dRin;
					Vec3f tmp;
					cvrm::decomposeRT(_dRin, dRin, tmp);
...
inline float Templates::pro_GP(const Matx33f& K, Pose& pose, const Mat1f& curProb, float thetaT, float errT)
{
	Rect curROI;
	int curView = this->_getNearestView(pose.R, pose.t);
	{
...
public:
	void loadModel(const std::string& modelFile, float modelScale, bool forceRebuild = false)
	{
		...
		auto tfile = ff::ReplacePathElem(modelFile, "tm033", ff::RPE_FILE_EXTENTION); //031(icosahedron) 032(0, 1, 0)
...
public:
	virtual void loadModel(const std::string& modelFile, const std::string&)
	{
	...
	virtual float update(Pose& pose)
	{
		...
		_cur.err = _obj.templ.pro(_K, _cur.pose, _cur.colorProb, thetaT, errT);
